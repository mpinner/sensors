...
void setup() {
  ...
  stripUpper.begin();
  stripUpper.show(); // Initialize all pixels to 'off'

  stripLower.begin();
  stripLower.show(); // Initialize all pixels to 'off'
}
...
uint32_t Wheel(byte WheelPos) {
  if(WheelPos < 85) {
    return stripUpper.Color(WheelPos * 3, 255 - WheelPos * 3, 0);
  } 
  else if(WheelPos < 170) {
    WheelPos -= 85;
    return stripUpper.Color(255 - WheelPos * 3, 0, WheelPos * 3);
  } 
  else {
    WheelPos -= 170;
    return stripUpper.Color(0, WheelPos * 3, 255 - WheelPos * 3);
  }
}
...
void makeUpperColor(int i, unsigned int hue) {

  if (hue > 255) hue = hue % 255;

  setUpperPixelColor(i, hue, myEnc.read() %255);
